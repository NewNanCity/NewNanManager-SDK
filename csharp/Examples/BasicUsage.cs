...
public class BasicUsage
{
    ...
    private static async Task PlayerManagementExample(NewNanManagerClient client)
    {
        ...
        // 创建新玩家
        var newPlayer = await client.Players.CreatePlayerAsync(
        ...
        var updatedPlayer = await client.Players.UpdatePlayerAsync(
            newPlayer.Id,
            new UpdatePlayerRequest { Discord = "example#1234" }
        );
    ...
    private static async Task TownManagementExample(NewNanManagerClient client)
    {
        ...
        // 创建新城镇
        var newTown = await client.Towns.CreateTownAsync(
            new CreateTownRequest
            {
                Name = "ExampleTown",
                Level = 1,
                Description = "示例城镇",
    ...
    private static async Task MonitoringExample(NewNanManagerClient client)
    {
        ...
        {
            // 发送心跳
            var heartbeat = await client.Monitor.HeartbeatAsync(
                server.Id,
    ...
    private static async Task TokenManagementExample(NewNanManagerClient client)
    {
        ...
        // 创建新Token
        var newToken = await client.Tokens.CreateApiTokenAsync(
        ...
        {
            // 更新Token
            var updatedToken = await client.Tokens.UpdateApiTokenAsync(
                newToken.TokenInfo.Id,
                new UpdateApiTokenRequest { Description = "更新后的示例Token" }
            );
