...
public class Program
{
    public static async Task Main(string[] args)
    {
        ...
        var baseUrl =
            args.Length > 0
                ? args[0]
                : Environment.GetEnvironmentVariable("NANMANAGER_BASE_URL")
                    ?? "https://your-server.com";
        var token =
            args.Length > 1
                ? args[1]
                : Environment.GetEnvironmentVariable("NANMANAGER_TOKEN") ?? "your-api-token";
    ...
    private static async Task RunExamples(NewNanManagerClient client)
    {
        ...
        try
        {
            ...
            {
                Console.WriteLine(
                    $"   - {token.Name} ({token.Role}) - 状态: {(token.Active ? "激活" : "禁用")}"
                );
            }
        }
        catch (Exception ex)
